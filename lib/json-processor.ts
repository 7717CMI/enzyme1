  ...
  for (let i = 0; i < allPaths.length; i += batchSize) {
    ...
    for (const { path: pathArray, data } of batch) {
      ...
      if (data.CAGR !== null && data.CAGR !== undefined) {
        ...
        // Calculate CAGR from 2025 to end year (2032)
        const cagrStartYear = allYears[0] + 5 // 2020 + 5 = 2025
...
export async function processJsonDataAsync(
  ...
  try {
    ...
    const baseYear = startYear + 5 // Base year = 2025 for 2020-2032 data
    // Historical/Forecast split: years before base year are historical
    const historicalEndYear = baseYear - 1 // 2024
